      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {isStaff && (
          <div className="mt-6 grid grid-cols-1 gap-4 border-t border-slate-100 pt-4 sm:grid-cols-2">
            ...
            <TeamSelect value={ticket.assigned_team_id} onChange={(teamId) => void patch({ assigned_team_id: teamId })} />
          </div>
        )}
      </div>
...
/** Select de equipe responsável (staff). Como em AssigneeSelect, não permite
 * "desatribuir" (o backend não suporta limpar assigned_team_id). */
function TeamSelect({ value, onChange }: { value: number | null; onChange: (id: number) => void }) {
  const { authCall } = useAuth()
  const loader = useCallback(() => authCall((t) => api.listTeams(t)), [authCall])
  const { data: teams } = useAsync(loader)
